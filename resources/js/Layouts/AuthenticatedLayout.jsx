...
export default function AuthenticatedLayout({ header, count, children }) {
    ...
    const [notificationSeens,setNotificationSeen] = useState(0);
    ...
    const setSeen=()=>{
        ...
        axios.post('/notification/seen',{data})
        .then(()=>console.log("seen successfully"))
        .catch((err)=>console.log(err));
    }

    const notificationSeen=async()=>{
        await axios.get('/seen-notification')
        .then((res)=>setNotificationSeen(res.data))
        .catch((err)=>console.log(err));
    }
    ...
    return (
        <div className="min-h-screen bg-gray-100 ">
            <div className="min-h-screen bg-gray-100 flex flex-row grow">
                {/* Sidebar - Hidden on mobile (sm and below) */}
                <div className={`sm:block hidden relative  min-h-full bg-white text-gray-900 transition-all duration-300 ease-in-out z-40 ${
                    ...
                    <div className="absolute right-[-8px] top-0 z-50">
                        <button
                ...
                <div className={` transition-all duration-300 ease-in-out grow`}>
                    {/* Top Navigation Bar */}
                                ...
                                <div className="relative hidden sm:ms-6 sm:flex sm:items-center gap-4">
                                    <div>
                                        {user.user_role !== 'admin' && user.user_role !== 'hr' ? (
                                            <>
                                                <Badge badgeContent={(hasUnread && notificationSeens) ? notificationSeens : 0} color="error">
                                                    <button
                                                        type="button"
                                                        className="text-gray-500 hover:text-gray-700 transition duration-150 ease-in-out"
                                                        onClick={toggleNotifications}
                                                    >
                                                        <NotificationsIcon onClick={setSeen} />
                                                    </button>
                                                </Badge>
                                                {showNotifications && (
                                            ...
                                            <>
                                                <Badge badgeContent={(read && notificationSeens) ? notificationSeens : 0} color="error">
                                                    <button
                                                        type="button"
                                                        className="text-gray-500 hover:text-gray-700 transition duration-150 ease-in-out"
                                                        onClick={toggleNotifications}
